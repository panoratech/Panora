...
@Injectable()
export class HrisConnectionsService {
  constructor(
    ...
    this.logger.setContext(HrisConnectionsService.name);
  }
  ...
  // WE WOULD HAVE CREATED A DEV ACCOUNT IN THE 5 Hriss (Panora dev account)
    ...
    try {
      ...
      throwTypedError(
        new ConnectionsError({
          ...
          message: 'HrisConnectionsService.handleHrisCallBack() call failed',
          cause: error,
        }),
        this.logger,
      );
    ...
    try {
      ...
      throwTypedError(
        new ConnectionsError({
          ...
          message:
            'HrisConnectionsService.handleHrisTokensRefresh() call failed',
          cause: error,
        }),
        this.logger,
      );
