...
@Injectable()
export class KeapConnectionService extends AbstractBaseConnectionService {
  ...
  async passthrough(
    ...
    try {
      ...
      return await this.retryService.makeRequest(
        ...
        'marketingautomation.keap.passthrough',
        config.linkedUserId,
      );
    } catch (error) {
      throw error;
  ...
  async handleCallback(opts: OAuthCallbackParams) {
    try {
      ...
      this.logger.log(
        'OAuth credentials : keap marketingautomation ' + JSON.stringify(data),
      );
      ...
      if (isNotUnique) {
        ...
        db_res = await this.prisma.connections.create({
          data: {
            ...
            vertical: 'marketingautomation',
            token_type: 'oauth2',
            account_url: CONNECTORS_METADATA['marketingautomation']['keap'].urls
              .apiUrl as string,
            access_token: this.cryptoService.encrypt(data.access_token),
            refresh_token: this.cryptoService.encrypt(data.refresh_token),
            expiration_timestamp: new Date(
