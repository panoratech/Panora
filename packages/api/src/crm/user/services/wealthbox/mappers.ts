import { WealthBoxUserInput, WealthBoxUserOutput } from './types';
import {
    UnifiedCrmUserInput,
...
@Injectable()
export class WealthBoxUserMapper implements IUserMapper {
  constructor(private mappersRegistry: MappersRegistry, private utils: Utils) {
    this.mappersRegistry.registerService('crm', 'user', 'wealthbox', this);
  }
  desunify(
    ...
    const result: WealthBoxUserInput = {
      name: source.name,
      email: source.email,
    };
  ...
  async unify(
    source: WealthBoxUserOutput | WealthBoxUserOutput[],
    connectionId: string,
    customFieldMappings?: {
      slug: string;
      remote_id: string;
