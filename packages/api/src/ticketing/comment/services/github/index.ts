...
export class GithubService implements ICommentService {
  ...
  async addComment(
    ...
    try {
      ...
      const githubTicketOutput = JSON.parse(
        remote_data.data,
      ) as GithubTicketOutput;

      if (
        githubTicketOutput.number &&
        githubTicketOutput.repository.name &&
        githubTicketOutput.repository.owner.login
      ) {
