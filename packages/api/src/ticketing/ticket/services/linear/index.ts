...
export class LinearService implements ITicketService {
  ...
  async sync(data: SyncParam): Promise<ApiResponse<LinearTicketOutput[]>> {
    try {
      ...
      const issueQuery = {
        query:
          'query { issues { nodes { id title description dueDate parent{ id } state { name } project{ id } labels { nodes { id } } completedAt priorityLabel assignee { id } comments { nodes { id } } } }}',
      };

      const resp = await axios.post(`${connection.account_url}`, issueQuery, {
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${this.cryptoService.decrypt(
            connection.access_token,
