...
export class AtsEeocs extends ClientSDK {
    ...
    async addEeocss(
        ...
        options?: RequestOptions
    ): Promise<operations.AddEeocssResponse> {
        const input$ = request;
        const headers$ = new Headers();
        headers$.set("user-agent", SDK_METADATA.userAgent);
        headers$.set("Content-Type", "application/json");
        ...
        const payload$ = schemas$.parse(
            input$,
            (value$) => operations.AddEeocssRequest$.outboundSchema.parse(value$),
            "Input validation failed"
        );
        const body$ = encodeJSON$("body", payload$.RequestBody, { explode: true });

        const path$ = this.templateURLComponent("/ats/eeocs/batch")();
        ...
        const [result$] = await this.matcher<operations.AddEeocssResponse>()
            .json(200, operations.AddEeocssResponse$, { key: "object" })
            .json(201, operations.AddEeocssResponse$, { key: "UnifiedEeocsOutputs" })
            .fail(["4XX", "5XX"])
            .match(response, request$, { extraFields: responseFields$ });
